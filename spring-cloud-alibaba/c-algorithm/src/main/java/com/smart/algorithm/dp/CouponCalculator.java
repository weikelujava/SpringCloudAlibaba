...
public class CouponCalculator {
    ...
    public static void recursion(List<Shop> shops,int index,List<List<ShopCoupon>> result){
        if(index>=shops.size()) return;
        Shop shop = shops.get(index);
        if(shop.getShopCoupons()==null || shop.getShopCoupons().isEmpty()){
            recursion(shops,++index,result);
            return;
        }
        if(result.isEmpty())
            for(ShopCoupon coupon : shop.getShopCoupons()) {
                result.add(new ArrayList<ShopCoupon>(){{add(coupon);}});
            }
        else {
            List<List<ShopCoupon>> copy = new ArrayList(){{//DEEP COPY
                for(List<ShopCoupon> list:result) {
                    add(new ArrayList<ShopCoupon>(list));
                }
            }};
            result.clear();
            //a/b  1/2 ->a1 a2 b1 b2
    ...
    //求组合,暂时有问题
    public static void recursionX(List<Shop> shops,int index,List<List<ShopCoupon>> result){
        if(index>=shops.size()) return;
        Shop shop = shops.get(index);
        if(shop.getShopCoupons()==null || shop.getShopCoupons().isEmpty()){
            recursionX(shops,++index,result);
            return;
        }
        List<List<ShopCoupon>> current = new ArrayList<>();
        List<List<ShopCoupon>> lastCopy = new ArrayList(){{//DEEP COPY
            for(List<ShopCoupon> list:result){
                add(new ArrayList<ShopCoupon>(list));
            }
        }};
        for(ShopCoupon coupon : shop.getShopCoupons()){
            //a/b/c/d  1/2/3/4 ->a1 a2 a3 a4,b*,c*,d*
            for(List<ShopCoupon> list: lastCopy){
                list.add(coupon);
            }
            current.add(new ArrayList(){{add(coupon);}});
        }
        result.addAll(current);
        result.addAll(lastCopy);
        recursionX(shops,++index,result);
    }
