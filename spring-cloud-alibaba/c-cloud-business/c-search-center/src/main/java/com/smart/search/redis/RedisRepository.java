...
public class RedisRepository {
     ...
     * key序列化
     */
    private static final Jackson2JsonRedisSerializer OBJECT_SERIALIZER;
    /**
    ...
    static {
        ...
        OBJECT_SERIALIZER = jackson2JsonRedisSerializer;
    }
     ...
     * @param value the value
     */
    public void setAndExpire(final String key, final Object value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> serializer = getRedisSerializer();
            byte[] keys = serializer.serialize(key);
            byte[] values = OBJECT_SERIALIZER.serialize(value);
            ...
            log.info("Redis缓存key: {}, 过期时间: {}", key, time);
            return 1L;
        });
    }
     ...
     * @param hashValue the hash value
     */
    public void putHashValueAndExpired(String key, String hashKey, final Object hashValue) {
        log.debug("[redisTemplate redis]  putHashValue()  key={},hashKey={},hashValue={} ", key, hashKey, hashValue);
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> redisSerializer = getRedisSerializer();
            byte[] rawKey = redisSerializer.serialize(key);
            byte[] rawHashKey = redisSerializer.serialize(hashKey);
            byte[] values = OBJECT_SERIALIZER.serialize(hashValue);
            connection.hSet(rawKey, rawHashKey, values);
            connection.expire(rawKey, 24 * 60 * 60);
            return 1L;
        });
    }
    ...
    /**
     * redis List数据结构 : 返回列表 key 中指定区间内的元素，区间以偏移量 start 和 end 指定, 然后删除start和end中的所有数据
     *
     * @param key
     * @param start
     * @param end
     * @return
     */
    public Object getListAndRemove(String key, int start, int end) {
        return redisTemplate.executePipelined((RedisCallback<List<byte[]>>) connection -> {
            RedisSerializer<String> redisSerializer = getRedisSerializer();
            List<byte[]> result = connection.lRange(redisSerializer.serialize(key), start, end);
            connection.lTrim(redisSerializer.serialize(key), end + 1, start - 1);
            return result;
        });
    }
    ...
    public void zSetAdd(String key){}
}
