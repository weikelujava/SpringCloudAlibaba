package com.smart.search.utils;//package com.hollysmart.pcenter.template;
...
public class RedisRepository {
     ...
     * value 序列化
     */
//    private static final JdkSerializationRedisSerializer FAST_JSON_REDIS_SERIALIZER = new JdkSerializationRedisSerializer();

    /**
     * 在使用JdkSerializationRedisSerializer对 redis中的值进行序列化后 redisManager看到的是二进制的数据
     * 修改成FastJsonRedisSerializer进行对value序列化
     */
    ...
    public RedisRepository(RedisTemplate<String, Object> redisTemplate) {
        ...
        this.redisTemplate.setValueSerializer(FAST_JSON_REDIS_SERIALIZER);
    }
    ...
    public void setExpire(final String key, final Object value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            byte[] values = FAST_JSON_REDIS_SERIALIZER.serialize(value);
    ...
    public void setExpire(final String[] keys, final Object[] values, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            for (int i = 0; i < keys.length; i++) {
                ...
                byte[] bValues = FAST_JSON_REDIS_SERIALIZER.serialize(values[i]);
    ...
    public Object get(final String key) {
        Object resultStr = redisTemplate.execute((RedisCallback<Object>) connection -> {
            ...
            return FAST_JSON_REDIS_SERIALIZER.deserialize(values);
        });
        log.debug("[redisTemplate redis]取出 缓存  url:{} ", key);
        return resultStr;
    }
        ...
        return redisTemplate.execute((RedisCallback<Map<String, Object>>) connection -> {
            ...
            if (CollectionUtils.isNotEmpty(keys)) {
                for (String key : keys) {
                    ...
                    Object value = FAST_JSON_REDIS_SERIALIZER.deserialize(bValues);
                    maps.put(key, value);
                }
            }
            return maps;
